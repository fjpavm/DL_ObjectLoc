...
def createTrainingForImage(in_imageName, in_annotations_by_image, in_classMap):
    ...
    return PosPrediction, SizePrediction, BBPrediction, ObjectPrediction, CountPrediction

class DatasetFromImageNames_Generator(keras.utils.Sequence) :

    def __init__(self, in_image_names, in_image_names_real, in_batch_size, in_annotations_by_image, in_annotations_by_image_real, in_classMap, in_separateBB=False) :
        self.m_image_names = in_image_names
        self.m_image_names_real = in_image_names_real
        self.m_batch_size = in_batch_size        
        self.m_annotations_by_image = in_annotations_by_image
        self.m_annotations_by_image_real = in_annotations_by_image_real
    ...
    def __getitem__(self, idx) :
        ...
        real_batchSize = syn_batchSize = self.m_batch_size/2
        if idx%2 == 0:
            real_batchSize = real_batchSize + self.m_batch_size%2
        else:
            syn_batchSize = syn_batchSize + self.m_batch_size%2

        batch_image_names = self.m_image_names[idx * real_batchSize : (idx+1) * real_batchSize]
        batch_image_names_real = self.m_image_names_real[idx * syn_batchSize : (idx+1) * syn_batchSize]
        ...
        for image_name in batch_image_names:
            ...
            PosPrediction, SizePrediction, BBPrediction, ObjectPrediction, CountPrediction = createTrainingForImage(image_name, self.m_annotations_by_image, self.m_classMap)
        ...
        for image_name in batch_image_names_real:
            source = self.m_annotations_by_image[image_name][0]['source']
            image_path = os.path.join(dataset_path[source], image_sub_path, image_name) 
            image = plt.imread(image_path)
            preprocessed = keras.applications.mobilenet_v2.preprocess_input(image)
            ...
            PosPrediction, SizePrediction, BBPrediction, ObjectPrediction, CountPrediction = createTrainingForImage(image_name, self.m_annotations_by_image_real, self.m_classMap)
            batch_Pos.append(PosPrediction)
            batch_Size.append(SizePrediction)
            batch_BBoxes.append(BBPrediction)
            batch_Objects.append(ObjectPrediction)
...
if __name__ == '__main__':
    ...
    real_csv_path = os.path.join(dataset_path['real'], challenge2_real_csv) 
    annotations_list_real, annotations_by_image_real, annotations_by_class_real = read_annotation_file(real_csv_path)
   ...
   # preprocessed image shape: (1080, 1920, 3)
    ...
    model.compile(optimizer='adam', 
                    ...
                    metrics={'BB_out':keras.metrics.RootMeanSquaredError(), 'obj_out':'sparse_categorical_accuracy'})
    ...
    boost_model = keras.Model(boost_input, [boost_pos_out, boost_size_out, boost_object_out, boost_class_out, boost_count_out], name='boost_model_original')
    boost_model.trainable = False
    boost_model_out = boost_model(out_concat)

    boost_model_reduced = keras.Model(boost_input, [boost_pos_out, boost_size_out, boost_object_out, boost_count_out], name='boost_model')
    boost_model_reduced.trainable = False
    boost_model_reduced_out = boost_model(out_concat)

    boosted_model = keras.Model(input_layer, boost_model_reduced_out) 

    boosted_model.compile(optimizer='adam', 
                    loss=['mse', 'mse', 'sparse_categorical_crossentropy', 'mse'], 
                    loss_weights=[1.0, 75.0, 10.0, 10.0, 10.0],
                    metrics={'boost_model':keras.metrics.RootMeanSquaredError(), 'boost_model_1':keras.metrics.RootMeanSquaredError(), 'boost_model_2':'sparse_categorical_accuracy'})


    if(not os.path.exists('test_names_real.json')):
        train_names_real, validate_names_real, test_names_real = separateData(annotations_by_image_real.keys())
        test_names_json = dict()
        test_names_json['name_list'] = test_names_real
        JsonUtils.writeJsonToFile('test_names_real.json',test_names_json)
    else:
        test_names_json = JsonUtils.readJsonFromFile('test_names_real.json')
        test_names_real = test_names_json['name_list']
        names_list_real = [image_name for image_name in annotations_by_image_real.keys() if image_name not in test_names_real]
        train_names_real, validate_names_real, tmp = separateData(names_list,numTest=0)
        print('remembered to exclude real test dataset')
    ...
    train_gen = DatasetFromImageNames_Generator(train_names, train_names_real,BATCH_SIZE,annotations_by_image, annotations_by_image_real, classMap)
    val_gen = DatasetFromImageNames_Generator(validate_names,validate_names_real,BATCH_SIZE,annotations_by_image,annotations_by_image_real, classMap)
    test_gen = DatasetFromImageNames_Generator(test_names,test_names_real,BATCH_SIZE,annotations_by_image,annotations_by_image_real, classMap)

    model_checkpoint=keras.callbacks.ModelCheckpoint('../weights/EdIntel_weights_'+trainType+'_{epoch:02d}.h5',save_weights_only=True)
    ...
    for epoch in range(MY_EPOCHS):
        ...
        train_names, validate_names, tmp = separateData(names_list,numTest=0)
        train_names_real, validate_names_real, tmp = separateData(names_list_real,numTest=0)
        train_gen = DatasetFromImageNames_Generator(train_names, train_names_real,BATCH_SIZE,annotations_by_image, annotations_by_image_real, classMap)
        val_gen = DatasetFromImageNames_Generator(validate_names,validate_names_real,BATCH_SIZE,annotations_by_image,annotations_by_image_real, classMap)
